  ...
  class poll
  {
	  ...
	  public function addPoll()
	  {
		  ...
		  if (empty($core->msgs)) {
			  ...
			  ($db->affected()) ? $hollysec->writeLog(PLG_PL_ADDED, "", "no", "module") . $core->msgOk(PLG_PL_ADDED) :  $core->msgAlert(_SYSTEM_PROCCESS);
	  ...
	  public function updatePoll()
	  {
		  ...
		  if (empty($core->msgs)) {
			  ...
			  ($res) ? $hollysec->writeLog(PLG_PL_UPDATED, "", "no", "module") . $core->msgOk(PLG_PL_UPDATED) :  $core->msgAlert(_SYSTEM_PROCCESS);
	  ...
	  function showPollQuestion()
	  {
		  ...
		  if (isset($_GET["result"]) == 1 || isset($_COOKIE["voted" . $id]) == 'yes') {
			  ...
			  if ($db->numrows($sql)) {
				  ...
				  print "<div><input type=\"submit\" value=\"".PLG_NS_NOW."\" class=\"votenow\" />\n";
				  print "<a href=\"" .  SITEURL . "/plugins/poll/get_poll.php?result=1\" id=\"viewresult\" class=\"votenow\">".PLG_NS_RESULT."</a></div>\n";
				  print "</form>\n";
				  print "</div>";
			  }
		  }
