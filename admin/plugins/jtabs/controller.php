...
<?php
  ...
  $title = sanitize($_POST['tabtitle']);
  print ($db->affected()) ? $hollysec->writeLog(PLG_JT_TAB .' <strong>'.$title.'</strong> '._DELETED, "", "no", "module") . $core->msgOk(PLG_JT_TAB .' <strong>'.$title.'</strong> '._DELETED) : $core->msgAlert(_SYSTEM_PROCCESS);
  endif;
?>