...
<?php
  ...
  if ($row['price'] == 0) {
      ...
      ($db->affected()) ? $hollysec->writeLog(_MEMBERSHIP . ' ' . $row['title'.$core->dblang] . _LG_MEM_ACTIVATED . $user->username, "user", "no", "user") . $core->msgOk(_MS_MEM_ACTIVE_OK . ' ' . $row['title'.$core->dblang], false) : $core->msgError(_SYSTEM_PROCCESS);
  } else {
      $form_url = HCODE . "gateways/" . $row2['dir'] . "/form.tpl.php";
