...
<?php
  ...
  if (isset($_POST['deleteSlide'])):
	...
	@unlink(HCODE . 'plugins/slideout/slides/'.$filename);
	$db->delete("plug_slideout", "id='" . (int)$id . "'");
	
	$title = sanitize($_POST['slidetitle']);
	print ($db->affected()) ? $hollysec->writeLog(PLG_SO_SLIDE .' <strong>'.$title.'</strong> '._DELETED, "", "no", "plugin") . $core->msgOk(PLG_SO_SLIDE .' <strong>'.$title.'</strong> '._DELETED) : $core->msgAlert(_SYSTEM_PROCCESS);
  endif;
?>