  ...
  class jQuerySlider
  {
	  ...
	  function processSliderImage()
	  {
		  ...
		  if (empty($core->msgs)) {
			  ...
			  if (!empty($_FILES['filename']['name'])) {
				  $filedir = HCODE . "plugins/jqueryslider/slides/";
	  ...
	  function updateConfiguration()
	  {
		  ...
		  if (empty($this->msgs)) {
			  ...
			  ($db->affected()) ? $hollysec->writeLog(PLG_JQ_CONF_UPDATED, "", "no", "plugin") . $core->msgOk(PLG_JQ_CONF_UPDATED) :  $core->msgAlert(_SYSTEM_PROCCESS);
		  } else
			  print $core->msgStatus();
	  }
