...
<?php
  ...
  if (isset($_POST['deleteSlide'])):
	...
	@unlink(HCODE . 'plugins/contentslider/slides/'.$filename);
	$db->delete("plug_content_slider", "id='" . (int)$id . "'");
	
	$title = sanitize($_POST['slidetitle']);
	print ($db->affected()) ? $hollysec->writeLog(PLG_CS_SLIDE .' <strong>'.$title.'</strong> '._DELETED, "", "no", "plugin") . $core->msgOk(PLG_CS_SLIDE .' <strong>'.$title.'</strong> '._DELETED) : $core->msgAlert(_SYSTEM_PROCCESS);
  endif;
?>