...
<?php
  ...
  if (isset($_POST['deleteSlide'])):
	...
	@unlink(HCODE . 'plugins/jqueryslider/slides/'.$filename);
	$db->delete("plug_slider", "id='" . $id . "'");
	
	$title = sanitize($_POST['slidetitle']);
	print ($db->affected()) ? $hollysec->writeLog(PLG_JQ_SLIDE .' <strong>'.$title.'</strong> '._DELETED, "", "no", "plugin") . $core->msgOk(PLG_JQ_SLIDE .' <strong>'.$title.'</strong> '._DELETED) : $core->msgAlert(_SYSTEM_PROCCESS);
  endif;
?>