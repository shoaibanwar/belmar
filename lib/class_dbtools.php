  ...
  class dbTools
  {
      ...
      function doBackup($fname = '', $gzip = true)
      {
          ...
          if (!($sql = $this->fetch())) {
              ...
              if ($db->affected()) {
			      $hollysec->writeLog(_BK_BACKUP_OK, "", "no", "database");
	  ...
	  function doRestore($fname){
		  global $db, $msgError, $hollysec;
		  
		  $filename = HCODE . 'admin/backups/'.trim($fname);
		  $templine = '';
		  $lines = file($filename);
		  ...
		  $hollysec->writeLog(_BK_RESTORE_OK, "", "no", "database");
      ...
      function fetch()
      {
          ...
          $dump .= '-- @package HollyCode CMS' . nl;
          $dump .= '-- @author HollyCode.com.' . nl;
