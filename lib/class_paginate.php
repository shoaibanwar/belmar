<?php
  ...
  class Paginator
  {
          ...
          if ($this->num_pages > 1) {
              ...
              for ($i = 1; $i <= $this->num_pages; $i++) {
                  ...
                  if ($i == 1 or $i == $this->num_pages or in_array($i, $this->range)) {
                      if ($i == $this->current_page) {
                          $this->display .= "<a href=\"#\" title=\"" . _PAG_GOTO . " $i " . _PAG_OF . " $this->num_pages\" class=\"number current tooltip\">$i</a>";
                      } else {
                          if ($path && $core->seo) {
                              $this->display .= "<a class=\"number tooltip\" title=\"" . _PAG_GOTO . " $i " . _PAG_OF . " $this->num_pages\" href=\"".$path.$i."-".$this->items_per_page.".html\">$i</a>";
                          } else {
                              $this->display .= "<a class=\"number tooltip\" title=\"" . _PAG_GOTO . " $i " . _PAG_OF . " $this->num_pages\" href=\"$_SERVER[PHP_SELF]?pg=$i&amp;ipp=$this->items_per_page$this->querystring\">$i</a>";
                          }
                      }
                  }
              ...
              for ($i = 1; $i <= $this->num_pages; $i++) {
                  if ($i == $this->current_page) {
					  ...
					  if ($path && $core->seo) {
						  $this->display .= "<a class=\"number\" href=\"".$path . $i."-".$this->items_per_page.".html\">$i</a>";
					  } else {
                          $this->display .= "<a class=\"number\" href=\"$_SERVER[PHP_SELF]?pg=$i&amp;ipp=$this->items_per_page$this->querystring\">$i</a>";
					  }
                  }
              }
          }
      ...
      public function items_per_page_gallary()
      {
          $items = '';
          $ipp_array = array(8);
          foreach ($ipp_array as $ipp_opt)
